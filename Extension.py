...
def main():
    ...
    # Extension filter, enhance, then crop numberplate
    box = (bbox_min_x - 10, bbox_min_y - 10, bbox_max_x + 10, bbox_max_y + 10)
    img = Image.open(input_filename)
    img = img.filter(ImageFilter.MedianFilter())
    enhancing = ImageEnhance.Contrast(img)
    img = enhancing.enhance(2)
    img = img.convert('1')
    crop = img.crop(box)
    crop = crop.resize((800, 200))
    crop.save("output_crop_image/numberplateCropped.png")

    #Extension read numberplate

    # Path to pytesseract install, modify if path is different from regular install
    path_to_tesseract = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
    image_path = "output_crop_image/numberplateCropped.png"
    pytesseract.tesseract_cmd = path_to_tesseract
    text = pytesseract.image_to_string(Image.open(image_path), lang="eng", config="--psm 10 --oem 3 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
    print(text)
